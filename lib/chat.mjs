...
function injectLiteralMatches(userInput, classics, ragPicked, maxInject = 4) {
  ...
  const picked = ragPicked.slice()
  const pickedIds = new Set(picked.map(c => c.id || `${c.book}|${c.quote}`))
  const injected = []

  // 收集 query 的 2~4 字片段，按长度从长到短匹配
  const segments = new Set()
  ...
  // 一次遍历 classics：找出"任一片段匹配"的
  for (const c of classics) {
    if (injected.length >= maxInject) break
    const cid = c.id || `${c.book}|${c.quote}`
    if (pickedIds.has(cid)) continue  // 已在候选里，不再注入
    const cq = norm(c.quote)
    if (!cq) continue
  ...
  // 字面匹配的放最前，rag 选的放后面
  return [...injected, ...picked].slice(0, Math.max(picked.length, injected.length + 16))
}

/**
 * RAG 选择：向量相似度 + MMR 多样性 + rotation + 字面包含 boost。
 * 失败时回退到规则匹配。
 */
export async function selectClassicsRag({ classics, userInput, apiKey, embeddingsUrl, dim = 1024, limit = 16, lambdaParam = 0.65, rotationPenalty = 0.3, perBookQuota = 3 }) {
  ...
  if (!matcher || !apiKey) {
    return selectClassics(classics, userInput, limit)
  }
  ...
  // 5. 字面包含 boost：保证强字面匹配的典籍一定在前 16 名
  return injectLiteralMatches(userInput, classics, ragPicked, 4)
}
...
export async function handleChat({ userInput, apiKey, model, classics, embeddingsUrl, embeddingDim = 1024, useRag = true }) {
  ...
  if (useRag) {
    ...
    candidates = selectClassics(classics, userInput, 16)
  }
