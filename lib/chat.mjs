 ...
 */
export async function loadRag(classics, { embeddingsUrl, dim = 1024, initialEmbeddings, userEmbeddings, removedIds }) {
  ...
  RAG_CACHE.loading = (async () => {
    try {
      ...
      if (initialEmbeddings) {
        ...
        // 填入未删除的初始向量
        let dst = 0
        // 我们需要 classics 的前 activeInitCount 项是初始项（且已过滤 removed）
        // 这里通过调用方传入的 classics 是已过滤的，但 initialEmbeddings 是原始顺序
        // 解决办法：依赖 classics 顺序为 [initial-active..., user-added...]
        // 初始 active 项数量 = classics.length - userCount
        const initActiveCount = classics.length - userCount
        for (let i = 0; i < initActiveCount; i++) {
          const c = classics[dst]
          // 通过 _initialIndex 判定：若 dst 处不是当前 i，说明 i 已被 removed，跳过
          if (!c || c._initialIndex !== i) continue
          embeddings.set(initialEmbeddings.subarray(i * dim, (i + 1) * dim), dst * dim)
          dst++
        }
        // 追加用户向量
 ...
 */
export async function selectClassicsRag({ classics, userInput, apiKey, embeddingsUrl, dim = 1024, limit = 16, lambdaParam = 0.65, rotationPenalty = 0.3, perBookQuota = 3, initialEmbeddings, userEmbeddings, removedIds }) {
  if (limit <= 0 || !classics?.length) return []

  // 1. 尝试加载 RAG
  const matcher = await loadRag(classics, { embeddingsUrl, dim, initialEmbeddings, userEmbeddings, removedIds })
  if (!matcher || !apiKey) {
...
export async function handleChat({ userInput, apiKey, model, classics, embeddingsUrl, embeddingDim = 1024, useRag = true, initialEmbeddings, userEmbeddings, removedIds }) {
  if (!apiKey) {
